...
func TestEvidencePoolUpdate(t *testing.T) {
	...
	block := types.MakeBlock(height+1, []types.Tx{}, lastExtCommit.ToCommit(), []types.Evidence{ev}, types.Proof{})
...
func initializeBlockStore(db dbm.DB, state sm.State, valAddr []byte) (*store.BlockStore, error) {
	...
	for i := int64(1); i <= state.LastBlockHeight; i++ {
		...
		block := state.MakeBlock(i, test.MakeNTxs(i, 1), lastCommit.ToCommit(), nil, state.Validators.Proposer.Address, types.Proof{})
