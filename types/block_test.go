...
func TestBlockAddEvidence(t *testing.T) {
	...
	block := MakeBlock(h, txs, extCommit.ToCommit(), evList, Proof{})
...
func TestBlockValidateBasic(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		t.Run(tc.testName, func(t *testing.T) {
			block := MakeBlock(h, txs, commit, evList, Proof{})
...
func TestBlockHash(t *testing.T) {
	...
	assert.Nil(t, MakeBlock(int64(3), []Tx{Tx("Hello World")}, nil, nil, Proof{}).Hash())
}
...
func TestBlockMakePartSet(t *testing.T) {
	...
	partSet, err := MakeBlock(int64(3), []Tx{Tx("Hello World")}, nil, nil, Proof{}).MakePartSet(1024)
	require.NoError(t, err)
...
func TestBlockMakePartSetWithEvidence(t *testing.T) {
	...
	partSet, err := MakeBlock(h, []Tx{Tx("Hello World")}, extCommit.ToCommit(), evList, Proof{}).MakePartSet(512)
	require.NoError(t, err)
...
func TestBlockHashesTo(t *testing.T) {
	...
	block := MakeBlock(h, []Tx{Tx("Hello World")}, extCommit.ToCommit(), evList, Proof{})
...
func TestBlockSize(t *testing.T) {
	size := MakeBlock(int64(3), []Tx{Tx("Hello World")}, nil, nil, Proof{}).Size()
	if size <= 0 {
...
func TestBlockString(t *testing.T) {
	...
	block := MakeBlock(int64(3), []Tx{Tx("Hello World")}, nil, nil, Proof{})
...
func TestBlockProtoBuf(t *testing.T) {
	...
	b1 := MakeBlock(h, []Tx{Tx([]byte{1})}, &Commit{Signatures: []CommitSig{}}, []Evidence{}, Proof{})
	b1.ProposerAddress = cmtrand.Bytes(crypto.AddressSize)

	b2 := MakeBlock(h, []Tx{Tx([]byte{1})}, c1, []Evidence{}, Proof{})
	...
	b3 := MakeBlock(h, []Tx{}, c1, []Evidence{}, Proof{})
