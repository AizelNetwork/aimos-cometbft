...
func TestBlockStoreSaveLoadBlock(t *testing.T) {
	...
	block := state.MakeBlock(bs.Height()+1, txs, new(types.Commit), nil, state.Validators.GetProposer().Address, types.Proof{})
...
func TestSaveBlockWithExtendedCommitPanicOnAbsentExtension(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			defer cleanup()
			h := bs.Height() + 1
			block := state.MakeBlock(h, test.MakeNTxs(h, 10), new(types.Commit), nil, state.Validators.GetProposer().Address, types.Proof{})
...
func TestLoadBlockPart(t *testing.T) {
	...
	block := state.MakeBlock(height, nil, new(types.Commit), nil, state.Validators.GetProposer().Address, types.Proof{})
	partSet, err := block.MakePartSet(types.BlockPartSizeBytes)
	require.NoError(t, err)
...
func TestLoadBlockMetaByHash(t *testing.T) {
	...
	b1 := state.MakeBlock(state.LastBlockHeight+1, test.MakeNTxs(state.LastBlockHeight+1, 10), new(types.Commit), nil, state.Validators.GetProposer().Address, types.Proof{})
...
func TestBlockFetchAtHeight(t *testing.T) {
	...
	defer cleanup()
	require.Equal(t, bs.Height(), int64(0), "initially the height should be zero")
	block := state.MakeBlock(bs.Height()+1, nil, new(types.Commit), nil, state.Validators.GetProposer().Address, types.Proof{})
